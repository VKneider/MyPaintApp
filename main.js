...
function init(){
    ...
    custom.type = "color";
    custom.classList = "prueba";

    for (let i = 0; i < colors.length; i++) {
        custom.addEventListener("input", () => {
          if (colors[i] == "custom") {
            color.style.background = custom.value;
            color.addEventListener("click", () => {
              CTX.strokeStyle = custom.value;
              CTX.fillStyle = custom.value;
            });
          }
        });
      ...
      let hola = document.querySelector(".container");
      hola.appendChild(custom);
